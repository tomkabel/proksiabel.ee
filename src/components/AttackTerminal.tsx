...
export default function AttackTerminal({ onSelectService }: AttackTerminalProps) {
  ...
  React.useEffect(() => {
    ...
    for (let i = 1; i <= command.length; i++) {
      timers.push(window.setTimeout(() => { if (!cancelled) setTyped(i); }, i * CHAR_MS));
    }
    let t = command.length * CHAR_MS + 300;
    for (let i = 1; i <= lines.length; i++) {
      const delay = FINDING_TO_SERVICE.includes(i - 1) ? FINDING_MS : LINE_MS;
      timers.push(window.setTimeout(() => { if (!cancelled) setRevealed(i); }, t));
      t += delay;
    }
    timers.push(window.setTimeout(() => { if (!cancelled) setDone(true); }, t + 400));
  ...
  return (
    <div ref={rootRef} className="relative">
      <div className="relative rounded-xl border border-white/10 bg-slate-950 shadow-2xl shadow-sky-950/40 overflow-hidden">
        ...
        {/* Scan output */}
        <div className="px-4 py-4 font-mono text-[13px] leading-6" role="log" aria-label={t.hero.terminal.title}>
          <div className="text-slate-300">
            <span className="text-teal-400">$</span> {command.slice(0, typed)}
            {!done && typed < command.length && (
              <span className="inline-block w-2 h-4 align-middle bg-teal-400 animate-caret ml-0.5" aria-hidden="true" />
            )}
          </div>
