...
async function optimizeImage(inputPath, outputDir) {
  ...
  try {
    ...
    execSync(`${imCommand} "${inputPath}" -quality 80 -resize 1920x1080^ -gravity center -extent 1920x1080 "${outputWebP}"`, {
      stdio: 'inherit'
    });
    ...
    console.log(`   Savings: ${((originalStats.size - webpStats.size) / 1024).toFixed(1)}KB (${((1 - webpStats.size / originalStats.size) * 100).toFixed(1)}%)`);
...
async function createResponsiveVariants(inputPath, outputDir) {
  ...
  for (const variant of variants) {
    ...
    try {
      execSync(`${imCommand} "${inputPath}" -quality 80 -resize ${variant.width}x${variant.height}^ -gravity center -extent ${variant.width}x${variant.height} "${outputPath}"`, {
        stdio: 'inherit'
      });
